...
class Simulation(object):
  ...
  def __init__(self, config):
    ...
    self.extract_plane = config.extract_plane
  ...
  def run(self):
    ...
    if self.sim_restore_iter > 0:
      if self.dataset == 'sailfish':
        ...
        #self.start_state = np.zeros(self.domain.sim_shape + [4])
        #self.start_boundary = np.zeros(self.domain.sim_shape + [1])
        #self.start_state = np.zeros(self.domain.sim_shape + [4]) + 3.1
        #self.start_boundary = np.zeros(self.domain.sim_shape + [1]) + 3.1
    ...
    cboundary = self.boundary_to_cboundary(boundary_encoder, 
                                           encoder_shape_converter)
    self.start_boundary = None
    #cstate = np.zeros((1,256,256,256,8))
    #cboundary = np.zeros((1,256,256,256,16))
    ...
    for i in xrange(self.num_iters):
      ...
      if (i) % self.sim_save_every == 0:
        # decode state
        if self.extract_plane and (self.DxQy.dims == 3):
          cstate_shape = cstate.shape
          lower_bound = max(0, cstate_shape[3]/2-32)
          upper_bound = min(cstate_shape[3], cstate_shape[3]/2+32)
          cstate_extract = cstate[:,:,:,lower_bound:upper_bound]
        else:
          cstate_extract = cstate
    ...
    if self.compare:
      if self.dataset == 'sailfish':
        ...
        for i in tqdm(xrange(self.num_iters+3)):
          subdomain = SubDomain([0, 0, 0], self.domain.sim_shape)
          if self.extract_plane:
            center = [x/2 for x in self.domain.sim_shape]
            size = [128, 128, 128]
            pos = [x - y/2 for x, y in zip(center, size)]
            subdomain = SubDomain(pos, size)
      ...
      for i in tqdm(xrange(self.num_iters)):
        if i % self.sim_save_every == 0:
          ...
          if self.dataset == 'sailfish':
            ...
            generated_vel, generated_rho = self.saver.read_vel_rho(i, add_batch=True)
            if self.extract_plane:
              center = [x/2 for x in generated_vel.shape[1:-1]]
              size = [128, 128, 128]
              pos = [x - y/2 for x, y in zip(center, size)]
              subdomain = SubDomain(pos, size)
              generated_vel = numpy_utils.mobius_extract_2(generated_vel, subdomain, has_batch=True)
              generated_rho = numpy_utils.mobius_extract_2(generated_rho, subdomain, has_batch=True)
            self.vis.update_compare_vel_rho(i, true_vel, true_rho, generated_vel, generated_rho)
  ...
  def cstate_to_vel_rho(self, decoder, decoder_shape_converter, cstate, cboundary):
    ...
    sim_shape = cstate.shape[1:-1]
    sim_shape = [x * pow(2, self.nr_downsamples) for x in sim_shape]
    [vel, rho] = self.mapping(decoder, [decoder_shape_converter, decoder_shape_converter], 
                              input_generator, sim_shape, 
                              self.input_shape)
    return vel, rho
